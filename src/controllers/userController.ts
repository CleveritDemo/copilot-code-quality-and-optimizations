...
export const create_admin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    let user = await userService.create_admin(name, email);    
...
export const create_project_manager = async (req: Request, res: Response) => {
  ...
  try {
    var user = await userService.create_project_manager(name, email);
    res.status(201).json(user);
  } catch (error) {
...
const {id} = req.params;
  ...
  try {
    let user = await userService.getUserById(Number(id));
  if (!user) {
